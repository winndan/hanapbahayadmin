...
def fetch_confirmed_bookings_today():
...
def fetch_average_bookings_per_month():
    ...
    timestamps = [parse(item["created_at"]).astimezone(timezone.utc) for item in response.data if "created_at" in item]
    if not timestamps:
        return 0
    
    first_booking = min(timestamps)
    months_active = max(1, (datetime.now(timezone.utc) - first_booking).days // 30)
...
confirmed_bookings_today = fetch_confirmed_bookings_today()
...
info_card_data = [
    ...
    ("Confirmed Today", f"{confirmed_bookings_today}", "Bookings confirmed today")
]
